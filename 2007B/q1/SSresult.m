...
endP=input('请输入终点：');
tic
%	calc_money(num_L,distance)  num_L表示是那一路线，diatance表示站点间隔数  结果为价钱
%  SS_matrix_con表示站点是否直接连通  数据来自SSdata_lian  为0就不通  为1就通zeros(3975)
load SSdata_lian;   
load SLdata;	%SLmat1行向量是站名 列向量是线路名  读取出来的数据表示在线路的哪一个站zeros(3975)
disp('------------------------------直达--------------------------------------')
direct=0;
result_mat=[];
%-----------
for i=1:520
    if SLmat1(startP,i,1)>0&&SLmat1(endP,i,1)>0&&(SLmat1(endP,i,1)-SLmat1(startP,i,1)>0)  %非环形
    ...
    if SLmat1(startP,i,2)>0&&SLmat1(endP,i,2)>0&&(SLmat1(endP,i,2)-SLmat1(startP,i,2)>0)   %环形
